...
In short, we now use this repository to generate PYTHIA 8 heavy-flavour samples, reduce the raw ROOT trees into subsampled analysis histograms, and make the current MONASH, JUNCTIONS, and CLOSEPACKING comparison plots. The preferred production is the unified heavy-flavour chain, where charm and beauty are generated in the same run and analyzed with AnalysisScripts/hf_mult_pt_analysis_multi.C. The paper THnSparse plotting path is documented in PlottingScripts/README.md. The split bbbar and ccbar chain remains available for independent reference samples and comparisons to older productions.
